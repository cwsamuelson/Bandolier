...
Example::Example()
  ...
  VBO->Layout() = Bandolier::BufferLayout{{ Bandolier::ShaderDataType::Float3, "a_Position" },
                                          { Bandolier::ShaderDataType::Float4, "a_Color" }, };

  mVAO->AddVertexBuffer(VBO);
  mVAO->SetIndexBuffer(IBO);

  std::vector<float> squareVertices{ -0.5f, -0.5f, 0.0f, 0.5f, -0.5f, 0.0f, 0.5f, 0.5f, 0.0f, -0.5f, 0.5f, 0.0f, };
  std::vector<uint32_t> squareIndices{ 0, 1, 2, 2, 3, 0 };

  std::vector<float> texSquareVertices{ -0.5f,
                                        -0.5f,
                                        0.0f,
                                        0.0f,
                                        0.0f,
...
bool Example::OnEvent(const Bandolier::Events::BaseEvent& e) {
  return false;
}
